...
int main(int argc, char *argv[]) {
    ...
    if (argc != 2) { fprintf(stderr, "usage: pracct filename"); exit(1); }
    if ((fp = fopen(argv[1], "r")) == NULL) { fprintf(stderr, "can't open %s", argv[1]); exit(1); }
    ...
    if (ferror(fp)) { fprintf(stderr, "read error"); exit(1); }
    exit(0);
}